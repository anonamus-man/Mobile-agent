...
import com.anonamus.mobileagent.MainActivity
import com.anonamus.mobileagent.R
...
class RuntimeExecutionService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action ?: ACTION_START) {
            ...
            ACTION_COMPLETE -> finishTask(
                ...
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Mobile Agent finished working in $projectName.",
                failed = false,
            )
            ACTION_FAILED -> finishTask(
                title = "Task needs attention",
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Mobile Agent could not finish the task.",
                failed = true,
            )
            ACTION_CANCELLED -> {
                taskRunning = false
    ...
    private fun runningNotification(detail: String, includeStop: Boolean): android.app.Notification {
        val builder = NotificationCompat.Builder(this, RUNNING_CHANNEL_ID)
            ...
            .setContentTitle("Mobile Agent is working")
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = getSystemService(PowerManager::class.java)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.anonamus.mobileagent:active-coding-task")
    ...
    companion object {
        const val ACTION_START = "com.anonamus.mobileagent.START_RUNTIME"
        const val ACTION_STOP = "com.anonamus.mobileagent.STOP_RUNTIME"
        const val ACTION_PROGRESS = "com.anonamus.mobileagent.PROGRESS_RUNTIME"
        const val ACTION_COMPLETE = "com.anonamus.mobileagent.COMPLETE_RUNTIME"
        const val ACTION_FAILED = "com.anonamus.mobileagent.FAIL_RUNTIME"
        const val ACTION_CANCELLED = "com.anonamus.mobileagent.CANCEL_RUNTIME"
        ...
        fun ensureNotificationChannels(context: android.content.Context) {
            ...
            manager.createNotificationChannel(
                NotificationChannel(RUNNING_CHANNEL_ID, "Running coding tasks", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Shows progress while Mobile Agent is working in the background"
                },
            )
            manager.createNotificationChannel(
